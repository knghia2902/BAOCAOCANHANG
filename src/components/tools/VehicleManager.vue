...
const handleSubmit = async () => {
    ...
    if (editingIndex.value === null) {
        ...
        await LogService.logAction('Thêm xe', 'Thêm xe mới: ' + plate);
    } else {
        // Edit existing
        ...
        if (currentVehicle) {
            ...
            await LogService.logAction('Sửa xe', 'Cập nhật xe: ' + plate);
        }
...
const handleImportExcel = async (e: Event) => {
    ...
    try {
        ...
        await LogService.logAction('Import xe', `Import Excel: ${importedCount} mới, ${updatedCount} cập nhật`);
    } catch (err) {
        console.error(err);
...
const handleExportExcel = async () => {
    ...
    try {
        ...
        await LogService.logAction('Xuất Excel xe', 'Xuất danh sách xe ra Excel');
    } catch (err) {
        console.error(err);
